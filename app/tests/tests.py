...
def test_access_faculty_dashboard_authenticated(client):
    username = "user1"
    password = "bar"
    user = User.objects.create_user(username=username, password=password, user_type='TEACHER')
    client.login(username=username, password=password)
    url = reverse('dashboard')
    response = client.get(url)
    assert response.status_code == 200
...
def test_access_admin_dashboard_authenticated(client):
    username = "user1"
    password = "bar"
    user = User.objects.create_user(username=username, password=password, user_type='ADMIN')
