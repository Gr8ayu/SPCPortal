...
from .models import Application, Offer, Company, DepartmentGroupEmail
...
from app.analytics import pie_branch_offer, pie_dream_open_offer
from app.analytics import department_wise_yearly_stats
...
def CheckElegibleForApplication(student, offer):
    ...
    if not is_elegible:
        errors.append(
            "Student is not elegible or already placed in open Dream Company")
    ...
    if (
        student.gender not in offer.eligible_gender
        and "OTHER" not in offer.eligible_gender
    ):
        is_elegible = False
...
class OfferListView(ListView):
    ...
    def get_queryset(self):
        ...
        if "eligible_only" in filters.keys():
            ...
            ids = []
            for q in qs:
                is_elegible, errors = CheckElegibleForApplication(
                    self.request.user.student, q
                )
                if is_elegible:
                    ids.append(q.id)
            print(len(qs))
    ...
    if request.user.is_admin_access():
        ...
        if request.method == "POST":
            ...
            html_message = render_to_string(
                "app/offer_email_template.html", {
                    "offer": offer, "domain_name": domain}
            )
            ...
            e = (
                DepartmentGroupEmail.objects.filter(graduation_year=batch)
                .filter(id__in=ids)
                .values("email")
            )
