...
class Student(models.Model):
    ...
    profile_pic = models.ImageField(
        upload_to="profile_pic", default="profile_pic/default_pic.png"
    )
    ...
    def is_placed(self):
        applications = Application.objects.filter(
            student=self, status="ACCEPTED", offer__offer_type="OPEN"
        )
...
class Education(models.Model):
    MODE = (("MGMT", "Management"), ("KCET", "KCET"), ("COMEDK", "COMEDK"))
    user = models.OneToOneField(
        Student, on_delete=models.CASCADE, related_name="education"
    )
    bio = models.TextField(blank=True)
    sem = models.IntegerField(default=1)

    admission_mode = models.CharField(
        max_length=10, choices=MODE, default="COMEDK")
    admission_mode_rank = models.IntegerField(null=True, blank=True)
    no_of_backlogs = models.IntegerField(default=0)
    department = models.ForeignKey(
        Department, on_delete=models.SET_NULL, null=True)
    X_board = models.CharField(
        max_length=20, help_text="10th Board name", blank=True)
    XII_board = models.CharField(
        max_length=20, help_text="12th/Diploma Board name", blank=True
    )
...
class Company(models.Model):
    COMPANY_CATEGORY = (
        ...
        ("R & D Media and Mobile Applications",
         "R & D Media and Mobile Applications"),
    ...
    logo = models.ImageField(
        upload_to="company_logo", default="company_logo/default.png"
    )
    category = models.CharField(
        max_length=40, choices=COMPANY_CATEGORY, default="Other"
    )
...
class Offer(models.Model):
    ...
    company = models.ForeignKey(
        Company, on_delete=models.CASCADE, related_name="offer")
    date = models.DateTimeField(default=datetime.datetime.now)
    deadline = models.DateTimeField()
    offer_type = models.CharField(
        max_length=20, default="DREAM", choices=OFFER_TYPE)
    category = models.CharField(
        max_length=20, default="INTERNSHIP", choices=CATEGORY)

    required_batch = models.IntegerField(null=True, blank=True)
    eligible_branches = models.ManyToManyField(
        Department, related_name="eligible_branches"
    )
    ...
    XII_cutoff = models.FloatField(
        help_text="12th/Diploma cutoff percentage", default=0
    )
    ...
    def accepted(self):
        return Application.objects.filter(offer=self).\
...
class Application(models.Model):
    ...
    status = models.CharField(
        max_length=10, choices=STATUS, default="PROCESSING")
...
def save_edu_contact_elegible(sender, instance, **kwargs):
